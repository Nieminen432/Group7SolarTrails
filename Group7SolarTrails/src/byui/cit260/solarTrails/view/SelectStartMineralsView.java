 ...
 * @author Hiatt-Adam
 */
public class SelectStartMineralsView extends View{
    ...
    private void choosePercent(double percent) {
        ...
        if (storageRemaining >= quantity) {
            ...
            for (int i = 0; i < Group7SolarTrails.inventory.size(); i++) {
                if (Group7SolarTrails.inventory.get(i).getInventoryType() == 3) {
            ...
            if (!isInitiated) {
                ...
                Group7SolarTrails.inventory.add(new InventoryItem(3, quantity, "Minerals"));
                //set amount loaded
            ...
            try {
                this.console.println("You have selected " + (int) (percent*10) + " percent Minerals.");
